...
def EstimGrad_W2dist(minibatch_S,minibatch_y_pred,minibatch_y_true,obs4histo_S,obs4histo_y_pred,obs4histo_y_true, NbBins=500,ID_TreatedVar=0,DistBetween='Predictions_errors',show_histos=False):
    """
    ...
    if show_histos:
        plt.plot(cumfreq_S0,'r',label="S0")
        plt.plot(cumfreq_S1,'b',label="S1")
        plt.xlim(490, 500)
        plt.legend()
        plt.show()
    ...
    return [WGradients,W_score,cumfreq_S0,cumfreq_S1]
    ...
    def forward(ctx,  y_pred, y, InfoPenaltyTerm):
        ...
        * InfoPenaltyTerm is the dictionary that contains the pertinent information to compute the regularization term and its gradient 
        ...
        if (InfoPenaltyTerm['lambda']>0.):
           [W_Gradients,W_score,cumfreq_S0,cumfreq_S1]=EstimGrad_W2dist(InfoPenaltyTerm['mb_S'],y_pred,y,InfoPenaltyTerm['o4h_S'],InfoPenaltyTerm['o4h_y_pred'],InfoPenaltyTerm['o4h_y_true'], NbBins=500,ID_TreatedVar=0,DistBetween=InfoPenaltyTerm['DistBetween']) #DistBetween = 'All_predictions' or 'Predictions_errors'
...
def LargeDatasetPred(model,var_X,BlockSizes,DEVICE='cpu'):
  ...
  while loc_miniBatch_Start<n_loc:
    ...
    #local prediction
    with torch.no_grad():
      minibatch=var_X[loc_miniBatch_Start:loc_miniBatch_End,:]
...
def W2R_fit(model,X_data,y_data, S, lambdavar, EPOCHS = 5, BATCH_SIZE = 32,obs_for_histo=1000,DistBetween='All_predictions',DEVICE='cpu'):
    ...
    while epoch<EPOCHS:
        ...
        while batch_start+BATCH_SIZE < n:
            ...
            var_X_batch = X_data[Curr_obsIDs,:].float()
            #var_y_batch = y_data[Curr_obsIDs,0].reshape(-1,1).float()  #added in the 1d case
            var_y_batch = y_data[Curr_obsIDs].reshape(-1,1).float() 
            ...
            var_X_4histo = X_data[obsIDs_4histo,:].float()
            var_y_4histo = y_data[obsIDs_4histo].reshape(-1,1).float()  #added in the 1d case
            with torch.no_grad():
